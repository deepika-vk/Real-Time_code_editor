...
function EditorPage() {
 ...
 useEffect(() => {
    const init = async () => {
      // Define handleErrors function before using it
      ...
      socketRef.current.on(ACTIONS.JOINED, ({ clients, username, socketId }) => {
        ...
        // Sync the current code with the newly joined client
        socketRef.current.emit(ACTIONS.SYNC_CODE, {
          code: codeRef.current || "", // Ensure that current code is sent
              socketId,
        });
      });
  ...
  const runCode = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/compile", {
        code: codeRef.current || "", // Use current code reference
 ...
 <div className="container-fluid vh-100 d-flex flex-column">
      <div className="row flex-grow-1">
        ...
        <div className="col-md-2 bg-dark text-light d-flex flex-column">
          ...
          <div className="d-flex flex-column flex-grow-1 overflow-auto" style={{ marginTop: "20px" }}>
        ...
        <div className="col-md-10 text-light d-flex flex-column">
          ...
          <Editor
            ...
            roomId={roomId}
            onCodeChange={(code) => {
              codeRef.current = code; // Update codeRef when the code changes
            }}
          />
        </div>
      </div>
